...
export default function Login() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      navigate(data.redirect || (data.user.role === 'admin' ? '/admin' : '/cashier'));
    } catch (err) {
  ...
  const handleContinueWithEmail = async (e) => {
    ...
    if (!String(username).includes('@')) {
      setError('Enter your registered email address to continue with email login');
      return;
    }
    setLoading(true);
    try {
    ...
    <div className="min-h-screen relative flex items-center justify-center bg-gradient-to-br from-teal-700 via-teal-600 to-cyan-700 px-4 py-8">
      ...
      <div className="bg-white rounded-2xl shadow-2xl p-6 sm:p-8 w-full max-w-md dark:bg-gray-800">
        <div className="text-center mb-8">
          ...
          <h1 className="text-2xl font-bold text-gray-800">Stream Pharmacy</h1>
          <p className="text-gray-500 text-sm mt-1">Cashier &amp; Sales Management System</p>
        </div>
